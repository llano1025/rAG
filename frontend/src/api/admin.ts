...
export const adminApi = {
  getUsers: async (page = 1, perPage = 20): Promise<PaginatedResponse<User>> => {
    return apiClient.get(`/api/admin/users?page=${page}&per_page=${perPage}`);
  },
  ...
  updateUser: async (id: string, updates: Partial<User>): Promise<User> => {
    return apiClient.put(`/api/admin/users/${id}`, updates);
  },
  ...
  toggleUserStatus: async (id: string): Promise<User> => {
    return apiClient.post(`/api/admin/users/${id}/toggle-status`);
  },
  ...
  createApiKey: async (name: string, permissions: string[]): Promise<{
    ...
    return apiClient.post('/api/admin/api-keys', { name, permissions });
  },
